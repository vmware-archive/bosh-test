...
var _ = Describe("Info", func() {
	...
	Context("failure cases", func() {
		...
		It("returns an error on an unexpected status code with body", func() {
			...
			_, err := client.Info()
			Expect(err).To(MatchError(ContainSubstring("Getting /info from director: Fetching info: Director responded with non-successful status code '502'")))
		})
	})
})
